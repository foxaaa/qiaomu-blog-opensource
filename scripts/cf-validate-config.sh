...
require_binding() {
  ...
  if ! grep -q "^[[:space:]]*${section_pattern}[[:space:]]*$" "${CONFIG_PATH}"; then
    echo "❌ Missing ${description} section in ${CONFIG_PATH}" >&2
    exit 1
  fi

  if ! grep -q "^[[:space:]]*${binding_pattern}[[:space:]]*$" "${CONFIG_PATH}"; then
...
if ! grep -Eq 'NEXT_PUBLIC_SITE_URL[[:space:]]*=[[:space:]]*"https?://[^"]+"' "${CONFIG_PATH}"; then
  echo "❌ Missing NEXT_PUBLIC_SITE_URL 应该 in ${CONFIG_PATH}" >&2
  exit 1
fi
